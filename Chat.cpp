...
std::string Chat::signUp()
{
	...
	if (_data_users.size())
	{
		...
		else
		{
			...
			std::cout << "Поздравляем, " << login << ", вы успешно зарегистрированы. \n";
		}
	}
	else
	{
		_data_users.insert({ login, password });
		std::cout << "Поздравляем, " << login << ", регистрация прошла успешно. \n";
	}
	return  login;
}
...
void Chat::messageReadPersonal(const std::string& login_from) {
	...
	if (_chat_message.size())
	{
		...
		for (auto& message : _chat_message)
		{
			...
			{
				std::cout << "От кого: " << message.getFrom() << "   >>  " << message.getText() << std::endl;
				count++;
			}
		}
...
void Chat::messageReadAll()
{
	if (_chat_message_all.size())
	{
		...
		{
			std::cout << "От кого:   " << message_all.getTo() << '\t' << " >>  "
